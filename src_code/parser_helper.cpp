...
ostream& operator<< (ostream& os, LibParserLUT& lut) {
  ...
  for (int i=0; i < lut.loadIndices.size(); ++i) {
    ...
    for (int j=0; j < lut.transitionIndices.size(); ++j)
      cout << lut.loadIndices[i] << "," << lut.transitionIndices[j] << "," << lut.tableVals[i][j] << "\n" ;
...
ostream& operator<< (ostream& os, LibParserTimingInfo& timing) {
  ...
  if ( timing.fromPin == "ck" ) { return os; }
  cout << "Timing info from " << timing.fromPin << "->" << timing.toPin << ": " << endl ;
  ...
  do {
    ...
    if (valid) {
      for (int i=0; i < pinNetPairs.size(); ++i) {
        cout << cellInst << "/" << pinNetPairs[i].first << "," << pinNetPairs[i].second << ":" << cellType << endl ;
      }
    }
...
//Store the cells into an array of LibParserCellInfo cell
bool get_cells_from_lib (string filename, std::unordered_map<string, LibParserCellInfo>& cellInstances) {
  ...
  do {
	  LibParserCellInfo cell;
	  valid = lp.read_cell_info (cell);
	  cellInstances[cell.name] = cell;
	  /*
	  if (cell.name == "vcc") { continue; } 
	  if (cell.name == "vss") { continue; } 
	  if (cell.name == "") { continue; } 
	  cout << "Cell Type: " << cell.name << "," << cell.leakagePower << endl;
	  for (int i = 0; i < cell.timingArcs.size(); i++) {
	  	cout << cell.timingArcs[i] << endl;
	  }
	  */
...
//Interpolate timing delay based on corresponding LUT
//Return timing delay
bool get_timing_delay (double& delay, LibParserCellInfo& cellInstance, string fromPin,
		string toPin, double capacitance, double inputSlew) {
  
  LibParserLUT delay_table;
  bool valid = false;
  for (const LibParserTimingInfo arc : cellInstance.timingArcs) {
  	if ((fromPin == arc.fromPin) && (toPin == arc.toPin)) {
		delay_table = arc.riseDelay;
		valid = true;
		break;
	}
  }
  ...
  if (!valid) {
	  cout << "an error occured while looking for LUT" << endl;
	  return false;
  }

  const vector<double>& index1Values = delay_table.loadIndices;   
  const vector<double>& index2Values = delay_table.transitionIndices;

  int index1_lower = 0;
  for(int i = 0; i < index1Values.size() - 1; i++) {
  	if (index1Values[i] <= capacitance) 
		index1_lower = i;
	else
		break;
  }
  ...
  int index1_upper = 0;
  for(int i = index1Values.size() - 1; i >= 0; i--) {
  	if (index1Values[i] >= capacitance)
		index1_upper = i;
	else
		break;
  ...
  if (index1_upper < index1_lower) {
	  cout << "your capacitance value doesn't fit into the LUT table" << endl;
  	  return false;
  }


  int index2_lower = 0;
  for(int i = 0; i < index2Values.size() - 1; i++) {
        if (index2Values[i] <= inputSlew)
                index2_lower = i;
        else
                break;
  }
  ...
  int index2_upper = 0;
  for(int m = index2Values.size() - 1; m >= 0; m--) {
        if (index2Values[m] >= inputSlew)
                index2_upper = m;
        else
                break;
  }

  if (index2_upper < index2_lower) {
          cout << "your input slew value doesn't fit into the LUT table" << endl;
          return false;
  }
  
  double wx = (inputSlew - index2Values[index2_lower])/(index2Values[index2_upper] - index2Values[index2_lower]);
  double wy = (capacitance - index1Values[index1_lower])/(index1Values[index1_upper] - index1Values[index1_lower]);

  const vector<vector<double>>& table = delay_table.tableVals;

  double upperLeft = table[index1_lower][index2_lower];
  double lowerLeft = table[index1_upper][index2_lower];
  double upperRight = table[index1_lower][index2_upper];
  double lowerRight = table[index1_upper][index2_upper];

  delay = (1 - wx) * (1 - wy) * upperLeft + wx * (1 - wy) * upperRight + 
	  (1 - wx) * wy * lowerLeft + wx * wy * lowerRight;

  return true;	
}

bool get_output_slew (double &slew, LibParserCellInfo cellInstance, string fromPin,
                string toPin, double capacitance, double inputSlew) {
  LibParserLUT slew_table;
  bool valid = false;
  for (LibParserTimingInfo x : cellInstance.timingArcs) {
        if ((fromPin == x.fromPin) && (toPin == x.toPin)) {
                slew_table = x.riseTransition;
                valid = true;
                break;
        }
  }
  ...
  const vector<double>& index1Values = slew_table.loadIndices;
  const vector<double>& index2Values = slew_table.transitionIndices;
  ...
  vector<vector<double>> table = slew_table.tableVals;
  ...
  slew = (1 - wx) * (1 - wy) * upperLeft + wx * (1 - wy) * upperRight +
          (1 - wx) * wy * lowerLeft + wx * wy * lowerRight;
...
int main(int args, char** argv) {
  ...
  if (argv[1] == string("lib")) {

    // cout << "Parsing lib..." << endl ;
    std::unordered_map<string, LibParserCellInfo> cellLibrary;
    bool libraryRetrieved = get_cells_from_lib (argv[2], cellLibrary) ;
    if (!libraryRetrieved) {
	cout << "An error occured while trying to extract cells from lib" << endl;
	return 1;
    }

    //prior info of cell instance
    string cellName = "in01s01";
    //look up cell object by name
    LibParserCellInfo arg = cellLibrary[cellName];

    string fromPin = "a";
    string toPin = "o";
    double inputSlew = 100.0;
    double capacitance = 20.0;
    
    double timingDelay;
    bool delayRetrieved = get_timing_delay (timingDelay, arg, fromPin, toPin, capacitance, inputSlew);
    if (!delayRetrieved) {
	cout << "An error occured while trying to retrieve timing delay" << endl;
	return 1;
    } else {
    	cout << cellName << "/" << fromPin << "->" << toPin << " : ";
	cout << "input trans = " << inputSlew << " load = " << capacitance ;
        cout <<	" delay = " << timingDelay << endl;
    }

    double timingDelayPlus;
    delayRetrieved = get_timing_delay (timingDelayPlus, arg, fromPin, toPin, capacitance, inputSlew + 50);
    if (!delayRetrieved) {
	cout << "An error occured while trying to retrieve timing delay" << endl;
	return 1;
    } else {
    	cout << cellName << "/" << fromPin << "->" << toPin << " : ";
	cout << "input trans = " << inputSlew + 50 << " load = " << capacitance ;
        cout <<	" delay = " << timingDelayPlus << endl;
    }

    double outputSlew;
    bool slewRetrieved = get_output_slew (outputSlew, arg, fromPin, toPin, capacitance, inputSlew);
    if (!slewRetrieved) {
    	cout << "An error occured while trying to retrieve output slew" << endl;
	return 1;
    } else {
    	cout << cellName << "/" << fromPin << "->" << toPin << " : ";
	cout << "input trans = " << inputSlew << " load = " << capacitance ;
        cout <<	" slew = " << outputSlew << endl;
    }

    float delta_D_over_delta_S = (timingDelayPlus - timingDelay)/50.0;
    cout << "Delta D / Delta S = " << delta_D_over_delta_S << endl;

  }
  else if (argv[1] == string("verilog")) {
